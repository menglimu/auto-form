...
<div class="ojb-form-box">
  <div class="ojb-form" :class="{required: required}">
    <span class="ojb-form-label" :style="{width: configAll.labelWidth}">{{config.label}}</span> 
    <!-- <el-button class="button-new-tag" size="small" @click="add">新建{{config.label}}</el-button>
    ...
    <div class="tag-box">
      <el-tag class="el-tag" :key="index" :class="[{'active':index == btnActive},'tag-'+validateResultArray[index]]" closable v-for="(item, index) in value" :disable-transitions="false" @click.native="changeTab(index)" @close="tagClose(index)"> 
      {{value[index][config.title] || config.label+(index+1)}}
      </el-tag>
      <el-button class="button-new-tag" size="small" @click="add">新建{{config.label}}</el-button>
      
      
      <transition name="fade" v-for="(item, index) in value" :key="index">
        <div class="form-box" v-show="index == btnActive">
